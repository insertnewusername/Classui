...
function insertCustomSettingsPanel() {
    ...
    modal.style.cssText = 'max-width: none; width: 100%; max-height: 600px; margin: 0; border: none; border-radius: 20px; padding: 0;';
    ...
    guidemenu.style.display = "grid";
    guidemenu.style.gridTemplateColumns = "1fr 1fr";
    ...
    updateBtn.className = 'feedback-btn update-check-btn';
    updateBtn.type = 'button';
    updateBtn.style.alignSelf = 'flex-start';
    ...
    feedbackBtn.addEventListener('mouseleave', () => (feedbackBtn.style.opacity = '0.85'));
    ...
    let updateAutoReloadScheduled = false;
    let updateSuccessResetTimer = null;

    function setUpdateButtonState(state) {
        updateBtn.classList.remove('is-success', 'is-loading', 'is-error');

        if (updateSuccessResetTimer) {
            clearTimeout(updateSuccessResetTimer);
            updateSuccessResetTimer = null;
        }
        ...
        if (state === 'error') {
            updateBtn.textContent = 'Connection Failed';
            updateBtn.classList.add('is-error');
            updateBtn.disabled = false;
            updateSuccessResetTimer = setTimeout(() => {
                if (!updateCheckBusy) setUpdateButtonState('idle');
            }, 2200);
            return;
        }
    ...
    function scheduleExtensionReload() {
        if (updateAutoReloadScheduled) return;
        updateAutoReloadScheduled = true;
        setTimeout(() => {
            try {
                if (typeof chrome !== 'undefined' && chrome.runtime && typeof chrome.runtime.sendMessage === 'function') {
    ...
    updateBtn.addEventListener('mouseleave', () => (updateBtn.style.opacity = '0.85'));
    updateBtn.addEventListener('click', async () => {
        ...
        const result = await requestImmediateExtensionUpdateCheck();
    ...
    newPanel.appendChild(toggleTodoBtn);
    newPanel.appendChild(toggleCalBtn);

    let decorationRowEl = null;
    profilePanel.insertAdjacentElement('afterend', tutorialPanel);
    tutorialPanel.insertAdjacentElement('afterend', newPanel);
    ...
    if (panelRoot) {
        (function setupDecorationSection() {
            ...
            decoRow.style.cssText = 'display:flex;flex-direction:column;gap:8px;margin-top:12px;';
            decorationRowEl = decoRow;

            const label = document.createElement('div');
            label.textContent = 'Decoration';
            label.style.cssText = 'font-size:16px;font-weight:600;';
            decoRow.appendChild(label);
            ...
            newPanel.appendChild(decoRow);
        })();

        const chooser = document.createElement('div');
        chooser.className = 'sidebar-chooser';
        chooser.style.cssText = 'display:flex;gap:20px;justify-content:center;align-items:flex-start;';

        const makeOption = (id, lightImgFile, darkImgFile, label) => {
            const outer = document.createElement('div');
            outer.id = id;
            outer.className = 'sidebar-option';
            outer.style.cssText = 'cursor:pointer;text-align:center;width:140px;border-radius:28px;padding:6px;box-sizing:border-box;corner-shape:squircle;';
            const frame = document.createElement('div');
            frame.style.cssText = 'width:125px;height:90px;border-radius:20px;corner-shape:squircle;overflow:hidden;display:block;flex-shrink:0;';
        ...
        toggleStack.style.cssText = 'display:flex;flex-direction:column;align-items:center;gap:8px;';
        toggleStack.appendChild(togglesBox);
        toggleStack.appendChild(toggleArchivedBtn);

        const separator = document.createElement('div');
        separator.style.cssText = 'width:1px;margin:6px 12px;align-self:stretch;';

        const separator2 = document.createElement('div');
        separator2.style.cssText = 'width:1px;margin:6px 12px;align-self:stretch;';
        ...
        sizeLabel.textContent = 'Sidebar Size';
        ...
        minusBtn.style.cssText = 'width:48px;height:40px;border-radius:14px;border:1px solid rgba(0,0,0,0.06);background:transparent;cursor:pointer;transition:opacity 0.2s ease;line-height:1;font-size:20px;font-weight:700;';

        const plusBtn = document.createElement('button');
        plusBtn.type = 'button';
        plusBtn.className = 'mgc-scale-btn mgc-scale-plus';
        plusBtn.textContent = '+';
        plusBtn.style.cssText = 'width:48px;height:40px;border-radius:14px;border:1px solid rgba(0,0,0,0.06);background:transparent;cursor:pointer;transition:opacity 0.2s ease;line-height:1;font-size:20px;font-weight:700;';
        ...
        containerRow.style.cssText = 'display:flex;justify-content:space-evenly;align-items:center;gap:12px;margin-top:-10px;flex-wrap: wrap;';
        containerRow.appendChild(toggleStack);
        containerRow.appendChild(separator);
        containerRow.appendChild(sizeRow);
        containerRow.appendChild(separator2);
        containerRow.appendChild(chooser);

        (function setupLayoutSection() {
            const layoutRow = document.createElement('div');
            layoutRow.className = 'layout-row';
            layoutRow.style.cssText = 'display:flex;flex-direction:column;gap:8px;margin-top:12px;';
            ...
            newPanel.appendChild(layoutRow);
        })();

        const sidebarSection = document.createElement('div');
        sidebarSection.className = 'sidebar-row';
        sidebarSection.style.cssText = 'display:flex;flex-direction:column;gap:8px;margin-top:20px;';

        const sidebarHeading = document.createElement('div');
        sidebarHeading.className = 'mgc-section-heading mgc-section-heading-sidebar';
        sidebarHeading.textContent = 'Sidebar';
        sidebarHeading.style.cssText = 'font-size:16px;font-weight:600;';
        ...
        sidebarAdjustmentsRow.style.cssText = 'display:flex;gap:12px;align-items:center;justify-content:center;padding:2px 12px 2px;width:100%;box-sizing:border-box;flex-wrap:wrap;';

        const iconStyleRow = document.createElement('div');
        iconStyleRow.className = 'sidebar-icon-style-row';
        iconStyleRow.style.cssText = 'display:flex;flex-direction:column;justify-content:space-evenly;gap:8px;align-items:center;padding:0;width:auto;box-sizing:border-box;';

        const iconStyleLabel = document.createElement('div');
        iconStyleLabel.textContent = 'Sidebar Icons';
        iconStyleLabel.style.cssText = 'font-size:13px;font-weight:500;';

        const iconStyleBtns = document.createElement('div');
        iconStyleBtns.style.cssText = 'display:flex;gap:8px;align-items:center;justify-content:flex-start;width:100%;';
        ...
        newIconsBtn.style.cssText = 'height:40px;border-radius:30px;padding:15px !important;justify-content:center;width:auto;min-width:0;';
        ...
        oldIconsBtn.style.cssText = 'height:40px;border-radius:30px;padding:15px !important;justify-content:center;width:auto;min-width:0;';
        ...
        hoverExpandRow.style.cssText = 'display:flex;flex-direction:column;gap:8px;align-items:flex-start;padding:0;width:auto;box-sizing:border-box;';

        const hoverExpandChooser = document.createElement('div');
        hoverExpandChooser.className = 'sidebar-chooser';
        hoverExpandChooser.style.cssText = 'display:flex;gap:12px;justify-content:center;align-items:flex-start;';

        const sidebarAdjustmentsDivider = document.createElement('div');
        sidebarAdjustmentsDivider.className = 'sidebar-adjustments-divider';
        sidebarAdjustmentsDivider.style.cssText = 'width:1px;align-self:stretch;min-height:102px;margin:6px 2px;';
        ...
        sidebarAdjustmentsRow.appendChild(sidebarAdjustmentsDivider);
        ...
        heightAdjustRow.style.cssText = 'display:flex;flex-direction:column;gap:8px;align-items:center;padding:12px;margin-bottom:12px;';

        const heightLabel = document.createElement('div');
        heightLabel.className = 'sidebar-height-label';
        heightLabel.textContent = 'Maximum Sidebar Height';
        ...
        function applyHomePageRedesignMode(enabled, persist = true) {
            const isEnabled = !!enabled;
            const isHomeRoute = /^https:\/\/classroom\.google\.com(\/u\/\d+)?(\/h)?\/?$/.test(window.location.href) || window.location.href === 'https://classroom.google.com/';

            try {
                if (document.body && isHomeRoute) {
                    document.body.classList.toggle('mc-homepage-redesign-enabled', isEnabled);
                }
            } catch (_) {}
        }
     ...
     // Declare these at the top of the outer function (or inside the if block)
...
const startInteraction = () => {
    ...
    if (currentSidebar) {
        ...
        // Guard against undefined sidebarHotspot
        if (typeof sidebarHotspot !== 'undefined' && sidebarHotspot) {
...
const endInteraction = () => {
    ...
    enrolledEls.forEach(el => el.style.setProperty('height', 'auto', 'important'));
        ...
        newPanel.appendChild(sidebarSection);
        ...
        function updateSeparatorColor() {
            const isDark = document.body.classList.contains('dark-mode');
            const c = isDark ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.08)';
            separator.style.background = c;
            separator2.style.background = c;
            sidebarAdjustmentsDivider.style.background = c;
        }
        ...
        try {
            if (decorationRowEl) {
                sidebarSection.insertAdjacentElement('afterend', decorationRowEl);
            }
        } catch (_) {}
...
overlayRow.className = 'layout-row';
overlayRow.style.cssText = 'display:flex;flex-direction:column;gap:8px;margin-top:12px;';

const overlayLabel = document.createElement('div');
overlayLabel.textContent = 'Background Overlay';
overlayLabel.style.cssText = 'font-size:16px;font-weight:600;';
overlayRow.appendChild(overlayLabel);

const overlayToggleContainer = document.createElement('div');
overlayToggleContainer.style.cssText = 'display:flex;gap:12px;align-items:center;';

const overlayToggle = document.createElement('div');
overlayToggle.className = 'overlay-toggle-track';
overlayToggle.id = 'overlay-toggle-track';
overlayToggle.style.cssText = 'position:relative;width:44px;height:24px;border-radius:12px;cursor:pointer;transition:0.3s;flex-shrink:0;background:#444;';
const overlayThumb = document.createElement('div');
overlayThumb.className = 'overlay-toggle-thumb';
overlayThumb.style.cssText = 'position:absolute;top:2px;left:2px;width:20px;height:20px;background:#fff;border-radius:50%;transition:0.3s;';
overlayToggle.appendChild(overlayThumb);

const overlayLabelText = document.createElement('span');
overlayLabelText.textContent = 'Enable tint overlay on background image';
overlayLabelText.style.cssText = 'font-size:14px;font-weight:500;';

overlayToggleContainer.appendChild(overlayLabelText);
overlayToggleContainer.appendChild(overlayToggle);
overlayRow.appendChild(overlayToggleContainer);

// Insert after Liquid Glass
if (typeof glassRow !== 'undefined' && glassRow) {
    glassRow.parentNode.insertBefore(overlayRow, glassRow.nextSibling);
} else {
    newPanel.appendChild(overlayRow);
}

// ---- Load initial state ----
chrome.storage.local.get('decoration:overlayEnabled', (data) => {
    const enabled = data['decoration:overlayEnabled'] !== undefined ? data['decoration:overlayEnabled'] : true;
    if (enabled) {
        overlayToggle.style.background = '#6c5ce7';
        overlayThumb.style.left = '22px';
    } else {
        overlayToggle.style.background = '#444';
        overlayThumb.style.left = '2px';
    }
});
...
overlayToggle.addEventListener('click', function() {
    chrome.storage.local.get('decoration:overlayEnabled', (data) => {
        const current = data['decoration:overlayEnabled'] !== undefined ? data['decoration:overlayEnabled'] : true;
        const newVal = !current;

        chrome.storage.local.set({ 'decoration:overlayEnabled': newVal }, () => {
            // Update UI immediately
            if (newVal) {
                this.style.background = '#6c5ce7';
                this.querySelector('.overlay-toggle-thumb').style.left = '22px';
            } else {
                this.style.background = '#444';
                this.querySelector('.overlay-toggle-thumb').style.left = '2px';
            }

            // ✅ FALLBACK: manually reapply the decoration if one is active
            // This ensures the overlay toggles even if the storage listener doesn't fire.
            if (window.currentDecoration) {
                window.applyDecorationFilename(window.currentDecoration, { persist: false });
            }
        });
    });
});
// ---- END BACKGROUND OVERLAY TOGGLE ----// ---- FONT PICKER ----
...
// END FONT PICKER
        ...
        function setSidebarMode(mode, persist = true) {
            ...
            try {
                ...
                classicOpt.style.borderRadius = '28px';
                floatingOpt.style.borderRadius = '28px';
                classicOpt.style.cornerShape = 'squircle';
                floatingOpt.style.cornerShape = 'squircle';
        ...
        applyHomePageRedesignMode(true, false);
